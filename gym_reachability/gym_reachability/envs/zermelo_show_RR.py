...
envType:
    ...
    'gap': wide target in middle, surrounded by obstacles
    'behind': target in middle, surrounded by obstacles, target behind
"""
...
class ZermeloShowRREnv(gym.Env):

  def __init__(
      self, device, mode='R', doneType='toEnd', thickness=.1,
      sample_inside_obs=False, envType='gap'
  ):
    ...
    if self.target2:
      self.target2_set_boundary = self.get_target2_set_boundary()
  ...
  def extend_state(self, states):
    ...
    for state in states:
      ...
      l2_x = self.target2_margin(state)
      new_states.append(np.append(state, max(l_x, l2_x)))
    return new_states
  ...
  def target_margin(self, s):
    ...
    if self.target:
      ...
      target_margin = np.min(np.array(l_x_list))
  ...
  def target2_margin(self, s):
    ...
    if self.target2:
      ...
      # target_set_safety_margin
      for _, target2_set in enumerate(self.target2_x_y_w_h):
        l2_x = calculate_margin_rect(s, target2_set, negativeInside=True)
        l2_x_list.append(l2_x)

      target2_margin = np.min(np.array(l2_x_list))

    else:
      target2_margin = self.target_margin(s)/self.scaling

    return self.scaling * target2_margin
  ...
  def get_target2_set_boundary(self):
    """Gets the target set boundary.
    ...
    Returns:
        np.ndarray: of the shape (#target, 5, 2). Since we use the box target
            ...
            axis consists of the (x, y) position.
    """
    num_target2_set = self.target2_x_y_w_h.shape[0]
    target2_set_boundary = np.zeros((num_target2_set, 5, 2))

    for idx, target2_set in enumerate(self.target2_x_y_w_h):
      x, y, w, h = target2_set
      x_l = x - w/2.0
      x_h = x + w/2.0
      y_l = y - h/2.0
      y_h = y + h/2.0
      target2_set_boundary[idx, :, 0] = [x_l, x_l, x_h, x_h, x_l]
      target2_set_boundary[idx, :, 1] = [y_l, y_h, y_h, y_l, y_l]

    return target2_set_boundary
  ...
  def get_warmup_examples(self, num_warmup_samples=100):
    ...
    for i in range(num_warmup_samples):
      ...
      l_x = self.target_margin(np.array([x, y]))
      l2_x = self.target2_margin(np.array([x, y]))
      heuristic_v[i, :] = np.maximum(l_x, l2_x)
      states[i, :] = x, y
  ...
  def get_value(self, q_func, nx=41, ny=121, addBias=False):
    ...
    while not it.finished:
      ...
      y = ys[idx[1]]
      l_x = self.target_margin(np.array([x, y]))
      l2_x = self.target_margin(np.array([x, y]))
      # g_x = self.safety_margin(np.array([x, y]))
      ...
      if addBias:
        v[idx] = q_func(state).min(dim=1)[0].item() + max(l_x, l2_x)
      else:
  ...
  def simulate_one_trajectory(
    ...
    for _ in range(T):
      if toEnd:
        ...
        #   result = -1  # failed
        #   break
        if self.mode in ["R"] and self.target_margin(state[:2]) <= 0:
          result = 1  # succeeded
          break
        if self.mode in ["RR"] and self.reached1 and self.reached2:
          result = 1  # succeeded
          break
  ...
  def plot_target_sets(
    ...
    if self.target2:
      for one_boundary in self.target2_set_boundary:
        ax.plot(
            one_boundary[:, 0], one_boundary[:, 1], color=c_t, lw=lw,
            zorder=zorder
        )
